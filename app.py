...
st.sidebar.header("V10.3 controls")
...
if mode == "Live prediction — frozen calibration":
    ...
    base_results = []
    state_key = f"v10.3_live::{calibration_label}::{table_name}"
    ...
    state_key = f"v10.3_history::{table_name}"
...
checks = [
    ...
    {"Check": "Walk-forward validation", "Status": "PASS" if len(dt_only(validation_source)) >= 50 else "LIMITED", "Detail": "Signals are generated from prior history only; live results are not used to validate the frozen calibration."},
...
# -----------------------------------------------------------------------------
st.divider()
st.subheader("🛡️ V10.3 app health")
health = [
    ...
    {"Check": "Future-data leakage", "Status": "PASS" if mode == "Live prediction — frozen calibration" else "PASS", "Detail": "Live results are excluded from frozen calibration validation; walk-forward tests use only prior observations."},
